            ...
            cursor = connection.execute(
                """INSERT INTO import_logs
                   (import_type,filename,user_id,success_count,skipped_count,
                    failed_count,errors_json) VALUES (?,?,?,?,?,?,?)""",
                (
...
class LegacyBusinessRepository:
    ...
    def person_matches(self, connection, name):
        return connection.execute(
            "SELECT id FROM people WHERE name=? ORDER BY id LIMIT 2", (name,)
        ).fetchall()

    def person_duplicate(self, connection, name, gender, company, id_last4, permit_last4):
        if id_last4:
            sql, values = "SELECT 1 FROM people WHERE name=? AND id_last4=?", (name, id_last4)
        elif permit_last4:
            sql, values = "SELECT 1 FROM people WHERE name=? AND permit_last4=?", (name, permit_last4)
        else:
            sql = """SELECT 1 FROM people WHERE name=? AND gender=?
    ...
    def quota_duplicate(self, connection, quota_type, company, approval_no, quota_no):
        if quota_no:
            row = connection.execute(
                "SELECT 1 FROM quotas WHERE quota_serial=? OR quota_number=? LIMIT 1",
                (quota_no, quota_no),
            ).fetchone()
        elif approval_no:
            row = connection.execute(
    ...
    def quota_id_by_number(self, connection, quota_no):
        return connection.execute(
            "SELECT id FROM quota WHERE quota_no=? LIMIT 1", (quota_no,)
        ).fetchone()
    ...
    def contract_duplicate(self, connection, contract_no):
        if not contract_no:
            return False
        return connection.execute(
            "SELECT 1 FROM contract WHERE contract_no=? LIMIT 1", (contract_no,)
